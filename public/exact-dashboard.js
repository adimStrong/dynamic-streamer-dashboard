...
// Socket event listeners for real-time Google Sheets updates
...
socket.on('dataUpdate', function(data) {
    console.log('Received Google Sheets data update:', data);
    
    // Merge Google Sheets data if available
    if (data.streamers && data.streamers.length > 0) {
        console.log('Updating with Google Sheets data...');
        
        // Replace the static data with live Google Sheets data
        streamData = data.streamers.map(item => ({
            date: item.date,
            views: item.views || 0,
        ...
        // Update filteredData to reflect the new data
        filteredData = [...streamData];
        
        console.log(`Updated with ${streamData.length} records from Google Sheets`);
    }
...
// Listen for successful Google Sheets connection to hide popup
socket.on('dataUpdate', (data) => {
    if (data && data.streamers && data.streamers.length > 0) {
        // If we receive real Google Sheets data, hide the popup
        const popup = document.getElementById('googleSheetsPopup');
        if (popup) {
            popup.classList.add('hidden');
            sessionStorage.setItem('popupDismissed', 'true');
